...
public class Archery extends Building{
	...
	public Archery(MainClass mc, int x, int y, Field f) {
		super(x,y,f,mc);
		currentSprite=Toolkit.getDefaultToolkit().getImage("Images/buildings/Archery.png");
	}
		...
		if(mc.isWithinBounds(mc.getMousePos(),topLeft,bottomRight)) {
			...
			if(mc.getCurrentPlayer().getGold().getAmount() >= 1250) {
				if (currentSquare.getHero().getUnitsAmmount() < 8) {
					...
					mc.getCurrentPlayer().getGold().removeAmount(1250);
				}		
			}
		}
		if (!clicked) {
