...
public class MageTower extends Building{
	...
	public MageTower(MainClass mc, int x, int y, Field f) {
		super(x,y,f,mc);
		currentSprite=Toolkit.getDefaultToolkit().getImage("Images/buildings/MageTower.png");
	}
	...
	public void draw(Graphics g) {
		...
		g.drawString("Buy Mage",155,205);
		g.drawString("Cost:",155,230);
		g.drawString("Gold: 2250",155,255);
	...
	public void mousePressed(MouseEvent e) {
		...
		if(mc.isWithinBounds(mc.getMousePos(),topLeft,bottomRight)) {
			clicked = true;
			if(mc.getCurrentPlayer().getGold().getAmount() >= 2250) {
				if (currentSquare.getHero().getUnitsAmmount() < 8) {
					...
					mc.getCurrentPlayer().getGold().removeAmount(2250);
				}		
			}
		}
		if (!clicked) {
